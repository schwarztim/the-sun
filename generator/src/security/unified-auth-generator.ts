...
export interface UnifiedAuthConfig {
  ...
  /**
   * Hermes integration mode. Defaults to "auto" — every SSO/OAuth MCP thesun
   * generates depends on the Hermes auth broker by default. With HERMES_URL +
   * HERMES_CLIENT_TOKEN set at runtime, tokens come from Hermes; otherwise the
   * generated standaloneAcquire() fallback runs the flow locally via
   * @hermes/auth-core. Pass "legacy" only to opt out into the old inlined auth.
   */
...
function generateHermesDualModeAuth(config: UnifiedAuthConfig): string {
  ...
  // standaloneAcquire runs ONLY when HERMES_URL is unset. It uses only verified
  // @hermes/auth-core APIs (createCredentialStore().get) plus env vars — the
  // Hermes broker is the canonical path and owns interactive SSO / refresh.
  let standaloneBody: string;
  if (primaryMethod === "azure_ad_sso") {
    standaloneBody = `
      // Local fallback: read a previously captured token from the Hermes
      // credential store or an env var. Interactive SSO is owned by the broker.
    ...
    standaloneBody = `
      // Local fallback: read a previously captured access token from the Hermes
      // credential store. OAuth refresh is owned by the broker.
      const { createCredentialStore } = await import('@hermes/auth-core');
      const store = await createCredentialStore();
      const stored = await store.get('${toolName}', 'access_token');
      const key = stored ?? process.env.${envPrefix}_ACCESS_TOKEN ?? process.env.${envPrefix}_TOKEN;
      if (!key) {
        throw new Error('No ${toolName} access token available. Configure HERMES_URL + HERMES_CLIENT_TOKEN (recommended), or set ${envPrefix}_ACCESS_TOKEN.');
      }
      return { accessToken: key, expiresAt: Date.now() + 3600_000 };`;
  } else {
...
export function generateUnifiedAuthCode(config: UnifiedAuthConfig): string {
  ...
  // Dual-mode (DEFAULT): generate thin wrapper using @hermes/auth-core.
  // Hermes is the canonical auth broker for every SSO/OAuth MCP thesun emits.
  if (hermesMode === "auto") {
    return generateHermesDualModeAuth({ ...config, hermesMode });
  }
