...
export function generateHARAuthCodeSnippet(toolName: string): string {
  ...
  return `
// HAR-based Authentication Support (self-contained, no external deps)
// Fallback path for webapps without an API. Hermes (@hermes/auth-core) is the
// preferred broker for SSO/OAuth targets; this HAR extractor covers the case
// where a token/cookie was captured manually from a HAR file or env var.
...
function loadHarAuth(): HarAuth {
  // 1. Pre-extracted token from env (populated by capture flow)
  const extractedToken = process.env.${envPrefix}_EXTRACTED_TOKEN;
  if (extractedToken) return { type: 'bearer', token: extractedToken };

  const extractedCookies = process.env.${envPrefix}_EXTRACTED_COOKIES;
  if (extractedCookies) return { type: 'cookie', cookie: extractedCookies };

  // 2. Extract from a HAR file on disk
  const harPath = process.env.${envPrefix}_HAR_FILE_PATH;
  if (harPath && existsSync(harPath)) {
    try {
      ...
      for (const entry of entries) {
        const authHeader = (entry?.request?.headers ?? []).find(
          (h: any) => String(h.name).toLowerCase() === 'authorization'
        );
        if (authHeader?.value) {
          const value = String(authHeader.value);
          return value.toLowerCase().startsWith('bearer ')
            ? { type: 'bearer', token: value.slice(7) }
            : { type: 'bearer', token: value };
        }
      }
      // Fall back to captured cookies if no Authorization header was found
      for (const entry of entries) {
        const cookieHeader = (entry?.request?.headers ?? []).find(
          (h: any) => String(h.name).toLowerCase() === 'cookie'
        );
        if (cookieHeader?.value) return { type: 'cookie', cookie: String(cookieHeader.value) };
      }
    } catch (err) {
      console.error('[Auth] Failed to parse HAR file:', (err as Error).message);
    }
  }
...
// Get auth headers from captured HAR/token/cookie data
export async function getAuthHeaders(): Promise<Record<string, string>> {
  const auth = loadHarAuth();
  const headers: Record<string, string> = {};
  ...
  if (auth.type === 'bearer') {
    ...
    throw new Error(
      'No authentication available. Set ${envPrefix}_EXTRACTED_TOKEN, ' +
      '${envPrefix}_EXTRACTED_COOKIES, or ${envPrefix}_HAR_FILE_PATH — ' +
      'or configure HERMES_URL + HERMES_CLIENT_TOKEN to use the Hermes broker.'
    );
  }
