...
export async function authenticate(): Promise<string> {
  ...
  const opts: DualModeAuthOptions = {
    ...
    standaloneAcquire: async () => {
      ...
      const key = stored ?? process.env.${envPrefix}_EXTRACTED_TOKEN ?? process.env.${envPrefix}_TOKEN;
      if (!key) {
...
export function generatePackageJsonAdditions(): {
  ...
  return {
    dependencies: {
      // Hermes is the canonical auth broker. The emitted robust-auth.ts imports
      // @hermes/auth-core getToken() (dual-mode); @hermes/client is the broker
      // transport it lazy-loads when HERMES_URL + HERMES_CLIENT_TOKEN are set.
