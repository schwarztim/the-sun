...
// ============================================================================
// Code Generation - Vault Store Module
// ============================================================================

/**
 * Emits the encrypted credential store into the generated server.
 *
 * This is emitted as a file rather than imported from a package because the
 * generated server must be installable on its own. It previously imported an
 * external dependency by local filesystem path, which resolved only on the
 * original author's machine and broke `npm ci` everywhere else.
 */
export function generateVaultStoreModule(): string {
  return `/**
 * Encrypted credential store (AES-256-GCM).
 *
 * Generated by thesun. Self-contained: no dependencies beyond the Node
 * standard library, so this server installs from a clean checkout.
 */
 ...
 * Otherwise a previously captured token is read from the Hermes credential
 * store, or straight from the environment when that package is not installed.
 *
 * @hermes/auth-core is an OPTIONAL dependency, loaded lazily on purpose. It
 * resolves from a private repository, so \`npm install\` cannot fetch it
 * everywhere. A static import would then fail at module load and take the whole
 * server down before it served a single tool, even though the environment
 * variable path below needs nothing from it. The specifier is held in a
 * variable so the compiler does not try to resolve it either.
 *
 * Generated by thesun with @hermes/auth-core dual-mode support.
 */

type HermesCredentialStore = { get(service: string, account: string): Promise<string | null> };
type HermesAuthCore = {
  getToken(opts: Record<string, unknown>): Promise<string>;
  createCredentialStore(): Promise<HermesCredentialStore>;
};
...
function envToken(): string | undefined {
  return process.env.${envPrefix}_EXTRACTED_TOKEN ?? process.env.${envPrefix}_TOKEN;
}

async function loadHermes(): Promise<HermesAuthCore | null> {
  const specifier = '@hermes/auth-core';
  try {
    return (await import(specifier)) as unknown as HermesAuthCore;
  } catch {
    return null; // not installed: fall back to the environment
  }
}
...
export async function authenticate(): Promise<string> {
  ...
  if (!hermes) {
    ...
    if (!key) {
      throw new Error(
        'No ${toolName} credential available, and @hermes/auth-core is not installed. ' +
        'Set ${envPrefix}_EXTRACTED_TOKEN, or install the broker and configure HERMES_URL + HERMES_CLIENT_TOKEN.',
      );
    }
    cachedToken = key;
  ...
  cachedToken = await hermes.getToken({
    service: '${toolName}',
    scheme: 'azure_ad_sso',
    // Local fallback (HERMES_URL unset): read a previously captured token from
    // the Hermes credential store or env. Interactive SSO is owned by the broker.
    standaloneAcquire: async () => {
      const store = await hermes.createCredentialStore();
      const stored = await store.get('${toolName}', 'access_token');
      const key = stored ?? envToken();
      if (!key) {
        throw new Error('No ${toolName} credential available. Configure HERMES_URL + HERMES_CLIENT_TOKEN (recommended), or set ${envPrefix}_EXTRACTED_TOKEN.');
      }
...
export function generateRobustAuthModule(
  config: AzureAdSsoConfig,
  hermesMode: "auto" | "legacy" = "auto",
): string {
  // Hermes dual-mode is the default for every Azure AD / SSO MCP thesun emits.
  if (hermesMode === "auto") {
...
async function getGlobalCredentials(realm: string): Promise<{ credential: GlobalSsoCredential; password: string } | null> {
  try {
    ...
    const password = await store.getPassword(GLOBAL_KEYCHAIN_SERVICE, cred.passwordKeychainKey);
...
async function saveGlobalCredentials(
  ...
  // Store password in global secure store
  await store.setPassword(GLOBAL_KEYCHAIN_SERVICE, keychainKey, password);
...
export function generatePackageJsonAdditions(): {
  ...
  return {
    ...
    // Hermes is the canonical auth broker, and these are OPTIONAL on purpose.
    // They resolve from a private repository, so as hard dependencies they made
    // `npm install` fail outright for anyone without access, which is the whole
    // server lost over an auth path many deployments never take. As optional
    // dependencies a failed fetch is not fatal, and the emitted robust-auth.ts
    // loads @hermes/auth-core lazily: with it present, broker and credential-
    // store modes work exactly as before; without it, the server still starts
    // and reads its token from the environment.
    optionalDependencies: {
      "@hermes/auth-core": "github:schwarztim/hermes#phase-0-1-impl",
      "@hermes/client": "github:schwarztim/hermes#phase-0-1-impl",
    },
    devDependencies: {
      "@types/prompts": "^2.4.9",
    },
...
export function generateAllAzureAdSsoFiles(
  ...
  return {
    ...
    "src/vault-store.ts": generateVaultStoreModule(),
