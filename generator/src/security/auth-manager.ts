...
export function generateAuthCodeSnippet(config: OAuthConfig): string {
  return `
// OAuth 2.0 Authentication for MCP Server (Hermes dual-mode)
// Generated by thesun - DO NOT STORE TOKENS HERE
//
// Hermes is the canonical auth broker. When HERMES_URL + HERMES_CLIENT_TOKEN
// are set, tokens come from the Hermes broker (refresh + reauth handled there).
// Otherwise standaloneAcquire() reads a captured access token from the local
// Hermes credential store via @hermes/auth-core.

import { getToken } from '@hermes/auth-core';
import type { DualModeAuthOptions } from '@hermes/auth-core';

const OAUTH_PROVIDER = '${config.provider}';
...
export async function authenticate(): Promise<string> {
  ...
  const opts: DualModeAuthOptions = {
    ...
    scheme: 'oauth',
    // Local fallback (HERMES_URL unset): read a previously captured access token
    // from the Hermes credential store or env. OAuth refresh is owned by the broker.
    standaloneAcquire: async () => {
      const { createCredentialStore } = await import('@hermes/auth-core');
      const store = await createCredentialStore();
      const stored = await store.get(OAUTH_PROVIDER, 'access_token');
      const key = stored ?? process.env.OAUTH_ACCESS_TOKEN;
      if (!key) {
        throw new Error(
          'No access token for ' + OAUTH_PROVIDER + '. Configure HERMES_URL + HERMES_CLIENT_TOKEN (recommended), or set OAUTH_ACCESS_TOKEN.'
        );
      }
      return { accessToken: key, expiresAt: Date.now() + 3600_000 };
    },
  };
