...
export function generateSecurityMiddleware(): string {
  return `
// Security Middleware for MCP Server (self-contained, no external deps)
...
const DANGEROUS_PATTERNS = [
  /\\$\\{.*\\}/,                 // template injection
  /<script[\\s>]/i,            // XSS
  /;\\s*(rm|del|drop|truncate)/i, // command/SQL chaining
  /\\.\\.\\//,                    // path traversal
  /(file|gopher|dict):\\/\\//i,  // SSRF protocols
];
...
/**
 * Validate that a token was issued FOR THIS SERVER (prevents token passthrough).
 * Decodes the JWT payload and checks the audience claim — signature verification
 * happens at the identity provider / Hermes broker, not here.
 */
function validateTokenIntegrity(
  ...
  try {
    ...
    const audiences = Array.isArray(payload.aud) ? payload.aud : [payload.aud];
    if (!audiences.includes(expectedAudience)) {
      return { valid: false, error: 'Token was not issued for this resource server (possible passthrough)' };
    }
    return { valid: true };
  } catch {
    return { valid: false, error: 'Token validation failed' };
...
function sanitizeInput(input: string, context: string): string {
  for (const pattern of DANGEROUS_PATTERNS) {
    if (pattern.test(input)) {
      throw new SecurityError(\`Input contains a dangerous pattern in \${context}\`, 'DANGEROUS_PATTERN');
    }
  }
  return input;
}
...
export function validateRequest(req: Request, token: string, expectedAudience: string): void {
  ...
  const tokenResult = validateTokenIntegrity(token, expectedAudience);
  ...
  for (const [key, value] of Object.entries(body)) {
    if (typeof value === 'string') {
      sanitizeInput(value, \`request.body.\${key}\`);
    }
  }
}
...
export function validateSession(sessionId: string, userId: string): void {
  ...
  if (parts.length !== 2 || parts[0] !== userId) {
    throw new SecurityError('Session ID user mismatch (possible hijacking)', 'SESSION_INVALID');
  }
}
`;
}
