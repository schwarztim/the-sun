...
interface UserClaudeConfig {
  ...
  pluginDirs: string[];
  home: string;
  /** Whether a Claude Code global config was found on this machine. */
  claudeDetected: boolean;
}

/**
 * Detect the user's Claude configuration in a platform-aware way.
 *
 * Claude Code writes its global config to `~/.claude.json`
 * (`%USERPROFILE%\.claude.json` on Windows — `homedir()` resolves both), so the
 * presence of that file is the portable "Claude is configured on this machine"
 * signal. This replaces the previous macOS/zsh-only `~/.zshrc` alias probe,
 * which never matched on Windows or non-zsh shells.
 */
function detectUserClaudeConfig(): UserClaudeConfig {
  ...
  // Platform-aware detection: ~/.claude.json (POSIX) / %USERPROFILE%\.claude.json (Windows).
  const claudeDetected = existsSync(join(home, '.claude.json'));
