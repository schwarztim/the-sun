...
export class PatternEngine {
   ...
   *
   * @param apiName Optional API/vendor name (e.g. toolSpec.vendor). When
   *   provided, a KNOWN_PATTERNS match enriches rate-limit detection with
   *   header names this API is known to use, filling in whatever the
   *   declared header parameters alone didn't surface.
   */
  analyzeEndpoints(endpoints: EndpointInfo[], apiName?: string): ApiPattern {
    ...
    // Detect rate limiting. detectRateLimiting only inspects header *names*
    // (values are unused — see its implementation above), so declared
    // header parameters can be fed directly even though we have no live
    // response to sample: build a name-only Record from headerParams.
    const headerDetected = this.detectRateLimiting(
      Object.fromEntries(headerParams.map((name) => [name, ""])),
    );

    let rateLimiting: RateLimitPattern = headerDetected;
    if (apiName) {
      const known = this.matchKnownPattern(apiName);
      if (known?.rateLimiting.hasRateLimiting) {
        // Known-pattern data fills in whatever header-based detection
        // missed; header-based detection (derived from this API's own
        // declared parameters) takes precedence on a field-by-field basis.
        rateLimiting = {
          hasRateLimiting: true,
          limitHeader: headerDetected.limitHeader ?? known.rateLimiting.limitHeader,
          remainingHeader:
            headerDetected.remainingHeader ?? known.rateLimiting.remainingHeader,
          resetHeader: headerDetected.resetHeader ?? known.rateLimiting.resetHeader,
          retryAfterHeader:
            headerDetected.retryAfterHeader ?? known.rateLimiting.retryAfterHeader,
        };
      }
    }
